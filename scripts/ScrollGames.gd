...
func _input(event: InputEvent) -> void:
	if event is InputEventKey:
		if event.is_action_pressed("right") and card_current_index < card_x_positions.size() - 1:
			card_current_index += 1
			scroll()
		if event.is_action_pressed("left") and card_current_index > 0:
			card_current_index -= 1
			scroll()


func _on_CardMenu1_pressed():
	get_tree().change_scene("res://scenes/TicTacToe_main.tscn")
	
func _on_CardMenu2_pressed():
	get_tree().change_scene("res://scenes/simon.tscn")
